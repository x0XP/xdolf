...
final class LegacyWorldVisuals implements FramePassManager.PassDefinition {
    ...
    static void assertSmokeRendered() {
        if(smokeLines==0||smokeBoxes==0||smokeTags==0)throw new IllegalStateException("World frame pass did not render all visual fixture types");
    }
    private static final RenderPipeline LINE_PIPELINE=RenderPipeline.builder(RenderPipelines.LINES_SNIPPET)
        .withLocation(id("pipeline/legacy_lines")).withDepthTestFunction(DepthTestFunction.NO_DEPTH_TEST)
    ...
    private static final RenderPipeline BOX_PIPELINE=RenderPipeline.builder(RenderPipelines.DEBUG_FILLED_SNIPPET)
        ...
        .withLocation(id("pipeline/legacy_boxes")).withDepthTestFunction(DepthTestFunction.NO_DEPTH_TEST)
        .withDepthWrite(false).withCull(false).withBlend(BlendFunction.TRANSLUCENT).build();
    private static final RenderType BOX=RenderType.create("xdolf_legacy_box",1536,false,false,BOX_PIPELINE,
        RenderType.CompositeState.builder().createCompositeState(false));
    private static final RenderPipeline TAG_PIPELINE=RenderPipeline.builder(RenderPipelines.DEBUG_FILLED_SNIPPET)
        .withVertexFormat(com.mojang.blaze3d.vertex.DefaultVertexFormat.POSITION_COLOR,com.mojang.blaze3d.vertex.VertexFormat.Mode.QUADS)
        .withLocation(id("pipeline/legacy_tag_background")).withDepthWrite(false).withCull(false)
        .withBlend(BlendFunction.TRANSLUCENT).build();
    private static final RenderType TAG_BACKGROUND=RenderType.create("xdolf_legacy_tag_background",1536,false,false,TAG_PIPELINE,
        RenderType.CompositeState.builder().createCompositeState(false));
    ...
    static void register() {
        AddFramePassEvent.BUS.addListener(event -> event.addPass(id("legacy_visuals"),new LegacyWorldVisuals()));
    }
    @Override public void targets(LevelTargetBundle targets,FramePass pass) {
        targets.main=pass.readsAndWrites(targets.main);
        var mc=Minecraft.getInstance();scene=extract(mc,mc.gameRenderer.getMainCamera().getPartialTickTime());
    }
    // Also implements the newer extraction overload; targets remains compatible with Forge 60.1.0.
    public void extracts(LevelTargetBundle targets,FramePass pass,DeltaTracker delta) {
        targets.main=pass.readsAndWrites(targets.main);scene=extract(Minecraft.getInstance(),delta.getGameTimeDeltaPartialTick(false));
    }
    @Override public void executes(LevelRenderState state) {
        ...
        try(var storage=new ByteBufferBuilder(65536)) {
            ...
            for(var tag:scene.tags) {
                ...
                LegacyGuiFont.drawWorld(buffers,matrix,tag.text,-half,tag.offset,0xFFFFFFFF);
                buffers.endBatch();
            }
    ...
    private static void line(MultiBufferSource.BufferSource buffers,Matrix4f matrix,Vec3 a,Vec3 b,int color,double width) {
        ...
        // The line shader uses the tangent to construct a constant physical-pixel width.
    ...
    private static void drawBox(MultiBufferSource.BufferSource buffers,Matrix4f matrix,Box box,Vec3 camera) {
        Vec3[] p=new Vec3[8];for(int i=0;i<8;i++)p[i]=corner(box,i).subtract(camera);
        // Six faces and twelve edges, including faces/edges hidden by terrain.
        ...
        for(int i=0;i<8;i++)for(int mask:new int[]{1,2,4})if(i<(i^mask))line(buffers,matrix,p[i],p[i^mask],LegacyVisualStyle.BOX_EDGE,LegacyVisualStyle.BOX_WIDTH);
    }
    ...
    private static Scene extract(Minecraft mc,float partial) {
        ...
        var camera=mc.gameRenderer.getMainCamera();var origin=camera.getPosition();
        if(Boolean.getBoolean("xdolf.smokeTest")&&smokeFixture)return fixture(origin,new Quaternionf(camera.rotation()));
        var segments=new ArrayList<Segment>();var boxes=new ArrayList<Box>();var tags=new ArrayList<Tag>();
        ...
        for(var entity:mc.level.entitiesForRendering()) {
            ...
            if(esp&&Hooks.espTarget(entity)) {
                ...
                var b=entity.getBoundingBox();boxes.add(new Box(new AABB(b.minX-0.05,b.minY,b.minZ-0.05,b.maxX+0.05,b.maxY+0.1,b.maxZ+0.05),0x26000000|color,null,0));
            }
            if(storage&&entity instanceof AbstractMinecart) {
                var type=entity.getType();int color=type==net.minecraft.world.entity.EntityType.CHEST_MINECART?0x00FF00:0xFFFFFF;
        ...
        if(storage||chestTracers) {
            ...
            for(int x=cx-radius;x<=cx+radius;x++)for(int z=cz-radius;z<=cz+radius;z++) {
                ...
                for(var be:loaded.getBlockEntities().values()) {
                    ...
                    if(be instanceof ChestBlockEntity&&state.hasProperty(BlockStateProperties.CHEST_TYPE)&&state.getValue(BlockStateProperties.CHEST_TYPE)!=ChestType.SINGLE) {
                        var other=pos.relative(ChestBlock.getConnectedDirection(state));
                        if(mc.level.getBlockEntity(other) instanceof ChestBlockEntity) {bounds=bounds.minmax(new AABB(other));merged.add(other.immutable());}
                    }
                    merged.add(pos.immutable());boxes.add(new Box(bounds,0x40000000|color,null,0));
                }
            }
        }
    ...
    /** Fixed visual objects for CI screenshots, drawn only in the opt-in development world. */
    private static Scene fixture(Vec3 camera,Quaternionf rotation) {
        ...
        var lines=new ArrayList<Segment>();var boxes=new ArrayList<Box>();var tags=new ArrayList<Tag>();
    ...
    private static void trajectory(Minecraft mc,float partial,List<Segment> lines,List<Box> boxes,Vec3 camera) {
        var stack=mc.player.getMainHandItem();boolean bow=stack.is(Items.BOW),rod=stack.is(Items.FISHING_ROD);
        ...
        charge=Math.min(1,(charge*charge+charge*2)/3);if(charge<=0.1)charge=1;
        var motion=mc.player.getLookAngle().normalize().scale(bow?charge*3:1.5);
        double gravity=bow?0.05:rod?0.15:0.03;
        var points=new ArrayList<Vec3>();
        var eye=mc.player.getEyePosition();
        for(int i=0;i<1000;i++) {
            points.add(position);position=position.add(motion);motion=motion.scale(0.99).add(0,-gravity,0);
        ...
        for(int i=1;i<points.size();i++)lines.add(new Segment(points.get(i-1),points.get(i),color,LegacyVisualStyle.TRAJECTORY_WIDTH));
