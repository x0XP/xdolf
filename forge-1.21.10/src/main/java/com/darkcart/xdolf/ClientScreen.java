...
final class ClientScreen extends Screen {
    ...
    private static void row(GuiGraphics g, String name, int x, int y, boolean enabled, boolean hover, boolean plus) {
        ...
        LegacyGuiFont.draw(g, name, x + 48 - LegacyGuiFont.width(name) / 2, y, color);
        if (plus) LegacyGuiFont.draw(g, "+", x + 90, y, enabled && hover ? 0xFF44AAFF : hover ? 0xFF888888 : 0xFFFFFFFF);
    }
    private static void draw(GuiGraphics g, Panel p, int mx, int my, boolean controls) {
        ...
        LegacyGuiFont.draw(g, p.title, p.x + 3, p.y + 1, 0xFFFFFFFF);
        if (controls) {
        ...
        for (int i = 0; i < p.sliders.size(); i++) {
            ...
            LegacyGuiFont.draw(g, slider.label + ": " + value, x + 1, y - 3, 0xFFFFFFFF);
        ...
        if (p.text()) {
            var lines = lines(p);
            for (int i = 0; i < lines.size(); i++) LegacyGuiFont.draw(g, LegacyGuiFont.trim(lines.get(i),97), p.x+3,p.y+13+i*10,0xFFFFFFFF);
        }
    }
    ...
    private static List<String> lines(Panel panel) {
        ...
        if (panel.title.equals("Radar")) {
            ...
            return players.stream().map(p -> (SocialState.isFriend(p.getName().getString()) ? "\u00a7a" : "\u00a7c") + p.getName().getString() + "\u00a7f: " + (int)p.distanceTo(player)).toList();
        }
            ...
            if (hit(mx,my,p.x+89,p.y+2,9,9)) p.open = !p.open;
                ...
                for (int i=0;i<p.modules.size();i++) if (hit(mx,my,p.x+2,p.y+12+i*12,96,11)) {
                    ...
                    if (button==0) ClientRuntime.toggle(module);
                        ...
                        if (!close) { var optionPanel=new Panel(label(module),0);optionPanel.x=0;optionPanel.open=true;optionPanel.temporary=true;optionPanel.options.addAll(options(module));PANELS.add(optionPanel); }
                    }
                    return true;
                }
                ...
                if (p.title.equals("Radar")) { int row=(int)((my-p.y-13)/10); var players=radar();
                    if (my>=p.y+13&&row>=0&&row<players.size()) { String name=players.get(row).getName().getString();SocialState.command(new String[]{"friend",SocialState.isFriend(name)?"remove":"add",name}); }
                }
            }
            return true;
        }
    ...
    @Override public boolean mouseDragged(MouseButtonEvent event,double dx,double dy) {
        ...
        if (sliding!=null) { moveSlider(event.x());return true; } return false;
    }
    private void moveSlider(double mx) {
        var s=sliding.setting;double fraction=Math.max(0,Math.min(1,(mx-offsetX)/90));
        double value=s.min+fraction*(s.max-s.min);if(sliding.integer)value=Math.floor(value);
        s.set(Math.max(s.min,Math.min(s.max,value)));
    }
    @Override public boolean mouseReleased(MouseButtonEvent event) { dragging=null;sliding=null;save();ClientConfig.save(ClientRuntime.MODULES);return true; }
    ...
    static void smokeCheckAndArrange() {
        ...
        if(values.sliders.size()!=13)throw new IllegalStateException("Expected thirteen original sliders");
    ...
    private static void save() {
        var file=FMLPaths.CONFIGDIR.get().resolve("xdolf-gui.properties"); var props=new Properties();
        for(var p:PANELS) if(!p.temporary) { props.setProperty(p.title+".x",""+p.x);props.setProperty(p.title+".y",""+p.y);props.setProperty(p.title+".open",""+p.open);props.setProperty(p.title+".pinned",""+p.pinned); }
        try { Files.createDirectories(file.getParent());try(var writer=Files.newBufferedWriter(file)) {props.store(writer,"Xdolf original click GUI window state");} }
        catch(java.io.IOException error) { LogUtils.getLogger().warn("Could not save Xdolf GUI",error); }
    }
}
