...
public class Minecraft implements IThreadListener, ISnooperInfo
{
            ...
            try
            {
                while (this.running)
                {
                    if (!this.hasCrashed || this.crashReporter == null)
                    {
                        ...
                        {
                        	if (player != null) {
								for (Module mod : Hacks.getEnabledHacks()) {
                                    mod.beforeUpdate(Wrapper.getPlayer());
								}
								for (Module mod : Hacks.getEnabledHacks()) {
                                    mod.onUpdate(Wrapper.getPlayer());
								}
								// this.sendMotionUpdates();
								for (Module mod : Hacks.getEnabledHacks()) {
                                    mod.afterUpdate(Wrapper.getPlayer());
								}
							}
                            this.runGameLoop();
                        }
    ...
    private void createDisplay() throws LWJGLException
    {
        ...
        Display.setTitle(Client.CLIENT_NAME + " " + Client.CLIENT_VERSION);
    ...
    private void runTickKeyboard() throws IOException
    {
        while (Keyboard.next())
        {
            ...
            if (flag)
            {
                ...
                //Wrapper.onKeyPressed(Keyboard.getEventKey(), Keyboard.getEventCharacter());
                Client.keyPressed(Keyboard.getEventKey());
