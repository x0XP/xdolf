...
public class FileManager
{
	...
	public void saveOptions()
	{
		try
		{
			File file = new File(xdolfDir.getAbsolutePath(), "options.txt");
			BufferedWriter out = new BufferedWriter(new FileWriter(file));
			for(Module mod: Hacks.getHacks())
			{
				if(mod.getOptions().size() > 0)
				{
					for(Option o : mod.getOptions()) {
						out.write(mod.getName().toLowerCase().replace(" ", "") + ":" + o.getName().toLowerCase().replace(" ",  "") + ":" + o.isEnabled());
						out.write("\r\n");
					}
				}
			}
	...
	public void loadOptions()
	{
		try
		{
			...
			while((line = br.readLine()) != null)
			{
				String curLine = line.toLowerCase().trim();
				String name = curLine.split(":")[0];
				String option = curLine.split(":")[1].replaceAll("\\s+","");;
				boolean isOn = Boolean.parseBoolean(curLine.split(":")[2]);
				for(Module mod: Hacks.getHacks())
				{
					if(mod.getName().toLowerCase().replace(" ", "").equals(name) && mod.getOptions().size() > 0)
					{
						for (Option o : mod.getOptions()) {
							if(o.getName().toLowerCase().replace(" ", "").equals(option))
								o.setState(isOn);
						}
					}
				}
	...
	public void loadWaypoints()
	{
		try
		{
			...
			while((line = br.readLine()) != null)
			{
				String curLine = line.trim();
				String name = curLine.split(":")[0];
				int dimension = Integer.parseInt(curLine.split(":")[1]);
				int x = Integer.parseInt(curLine.split(":")[2]);
				int y = Integer.parseInt(curLine.split(":")[3]);
				int z = Integer.parseInt(curLine.split(":")[4]);
				
				Waypoint point = new Waypoint(name, dimension, x, y, z, RenderManager.renderPosX, RenderManager.renderPosY, RenderManager.renderPosZ);
			}
			br.close();
		}catch(Exception e)
		{
