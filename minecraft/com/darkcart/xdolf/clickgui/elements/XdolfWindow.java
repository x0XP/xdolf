...
public class XdolfWindow {
	...
	private ArrayList<XdolfCheckbox> checkList = new ArrayList<XdolfCheckbox>();
	...
	public void draw(int x, int y) {
		...
		RenderUtils.drawBorderedRect(getXAndDrag(), getYAndDrag(), getXAndDrag() + 100, getYAndDrag() + 13 + (isOpen ? (12 * buttonList.size() + 0.5F) + (12 * checkList.size()) + (19 * sliderList.size() + (sliderList.size() != 0 ? 2.5F : 0)) : 0) + toAdd, 0.5F, 0xFF000000, 0x80000000);
		...
		if(isOpen) {
			...
			for(XdolfCheckbox c : checkList) {
				c.draw();
				if(x >= c.getX() + dragX && y >= c.getY() + dragY && x <= c.getX() + 96 + dragX && y <= c.getY() + 11 + dragY) {
					c.overButton = true;
				} else {
					c.overButton = false;
				}
					...
					if(x >= getXAndDrag() && y >= getYAndDrag()+ (10 * count) + 2 && x <= getXAndDrag() + 90 && y <= getYAndDrag() + (10 * count) + 13 + 2)
					{
						for(Friend friend: Wrapper.getFriends().friendsList)
						{
							if(Wrapper.getFriends().isFriend(e.getName()))
							{
								...
								Wrapper.getFriends().addFriend(e.getName());
								Wrapper.addChatMessage("Added " + e.getName() + " to friends.");
								Wrapper.getFileManager().saveFriends();
							}
						}
						Wrapper.getFileManager().saveFriends();
	...
	public void addCheckbox(Option option) {
		checkList.add(new XdolfCheckbox(this, option, x + 2, y + 12 + (12 * checkList.size())));
	}
	...
	protected void loadButtonsFromOptions(Module m) {
