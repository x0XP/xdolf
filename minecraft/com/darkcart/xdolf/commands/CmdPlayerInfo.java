		...
		for (int x = 0; x < Wrapper.getWorld().loadedEntityList.size(); x++)
		{
			if (Wrapper.getWorld().loadedEntityList.get(x) instanceof EntityPlayer)
			{
				...
				if (ent.getName().equalsIgnoreCase(name))
				{
					...
					Wrapper.addChatMessage("Health: " + (int)((ent.getHealth() / 20) * 100) + "%");
					Wrapper.addChatMessage("Distance: " + Math.abs(ent.getDistanceToEntity(Wrapper.getPlayer())));
					if (ent.inventory.armorItemInSlot(3).isItemEnchanted())
					{
					...
					if (ent.inventory.armorItemInSlot(2).isItemEnchanted())
					{
					...
					if (ent.inventory.armorItemInSlot(1).isItemEnchanted())
					{
					...
					if (ent.inventory.armorItemInSlot(0).isItemEnchanted())
					{
						ArrayList var18 = new ArrayList();
