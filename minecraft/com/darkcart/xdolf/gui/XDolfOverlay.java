...
import com.darkcart.xdolf.commands.CmdHide;
...
public class XDolfOverlay extends GuiIngame {
	...
	public void renderXDolfOverlay() {
		...
		if (Wrapper.getMinecraft().gameSettings.showDebugInfo || hideGui
				|| Wrapper.getMinecraft().currentScreen instanceof GuiChat)
			return;

		int width = Client.gameResolution.getScaledWidth();
		int height = Client.gameResolution.getScaledHeight();

		//if (CmdHide.displayLogo) {
		//	Fonts.roboto18.drawStringWithShadow("Xdolf", 2, 0, 0x55FF55);
		//}

		if (CmdHide.displayEnabledMods) {
			int count = 0;
			try {
				for (Module mod : Hacks.getDisplayedHacks()) {
		...
		if (CmdHide.displayPotions) {
			...
			if (!var4.isEmpty()) {
				...
				for (PotionEffect potioneffect : Ordering.natural().sortedCopy(var4)) {
					...
					Fonts.roboto18.drawStringWithShadow(s1, var14, height - (count2 * 20), 16777215);
					var14 = width - Fonts.roboto18.getStringWidth(var11) - 2;
					Fonts.roboto18.drawStringWithShadow(var11, (var16) - 8, height + 10 - (count2 * 20), 8355711);
				}
			}
		}
